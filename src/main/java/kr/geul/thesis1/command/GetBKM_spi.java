...
public class GetBKM_spi extends Command {
	...
	public GetBKM_spi(ArrayList<String> arguments) {
		super(arguments);	
	}
			...
			for (int month = monthBegin; month < monthEnd; month++) {
				...
				for (int part = 0; part < numberOfParts; part++) {
					...
					if (arguments.size() == 1 && arguments.get(0).length() == 6)
						enterCommand("bkms_estimate " + fileName + " 20" + arguments.get(0) + ";");
					else
						enterCommand("bkms_estimate " + fileName + ";");			
