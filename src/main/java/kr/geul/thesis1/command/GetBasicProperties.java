...
public class GetBasicProperties extends Command {
	...
	public GetBasicProperties(ArrayList<String> arguments) {
		super(arguments);	
	}
	...
	protected void runCommand() throws Exception {
		...
		for (int year = yearBegin; year < yearEnd; year++) {
			...
			for (int month = monthBegin; month < monthEnd; month++) {
				...
				enterCommand("getb_log " + isRunOnServer + " " + year + " " + month + ";");
				...
				for (int part = 0; part < numberOfParts; part++) {
					...
					enterCommand("getb_filter;");	
					
					if (arguments.size() == 1 && arguments.get(0).length() == 6)
						enterCommand("getb_get 20" + arguments.get(0) + ";");
					else
						enterCommand("getb_get;");
