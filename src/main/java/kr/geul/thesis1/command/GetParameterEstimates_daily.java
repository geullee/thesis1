...
public class GetParameterEstimates_daily extends Command {
	...
	public GetParameterEstimates_daily(ArrayList<String> arguments) {
		super(arguments);	
	}
	...
	protected void runCommand() throws Exception {
		...
		for (int year = yearBegin; year < yearEnd; year++) {
			...
			for (int month = monthBegin; month < monthEnd; month++) {
				...
				enterCommand("ped_log " + isRunOnServer + " " + year + " " + month + ";");						
				...
				for (int part = 0; part < numberOfParts; part++) {
					...
					if (arguments.size() >= 1) {
						...
						if (arguments.size() == 1 && arguments.get(0).length() == 6)
							enterCommand("ped_estimate " + fileName + " 20" 
									+ arguments.get(0) + ";");
						
						else if (arguments.size() == 6 && arguments.get(0).length() == 6)
							enterCommand("ped_estimate " + fileName + " 20" 
									+ arguments.get(0) + " " 
									+ arguments.get(1) + " " 
									+ arguments.get(2) + " " 
						...
						else if (arguments.size() == 9 && arguments.get(0).length() == 6)
							enterCommand("ped_estimate " + fileName + " 20" 
									+ arguments.get(0) + " " 
									+ arguments.get(1) + " " 
									+ arguments.get(2) + " " 
							...
							enterCommand("ped_estimate " + fileName + ";");
						}
